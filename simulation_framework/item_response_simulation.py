...
class item_response_simulation():
    ...
    def set_up(self, q_structure="custom", early_Q=np.empty(0), late_Q=np.empty(0),
               ensure_id=False, q_share=0, sigma_constraint="none", gain_mean=1):
        ...
        self.person_parameters = self.respondend_population.initialize_random_person_parameters(
            early_Q=early_Q, late_Q=late_Q, q_share=q_share, constraint_type=sigma_constraint, late_mean_value=gain_mean)
        ...
        # TODO: Merge this with initialize_item_parameters
        self.response_simulation.set_sigma_psi(sigma_psi=self.person_parameters["covariance"])
